...
export async function listar(institucionId, { solo_activos = true, categoria_id } = {}) {
  ...
  let i = 2;
  if (solo_activos) { conditions.push(`p.activo = true`); }
  if (categoria_id) { conditions.push(`p.categoria_id = $${i++}`); params.push(categoria_id); }

  const { rows } = await pool.query(
    `SELECT p.*, c.nombre AS categoria_nombre, s.cantidad AS stock_actual
     FROM productos p
...
export async function crear(institucionId, data) {
  const { categoria_id, nombre, descripcion, precio, genera_ticket = false, stock_inicial = 0 } = data;
  ...
  try {
    ...
    const { rows } = await client.query(
      `INSERT INTO productos (institucion_id, categoria_id, nombre, descripcion, precio, genera_ticket)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
      [institucionId, categoria_id, nombre, descripcion || null, precio, genera_ticket]
    );
    await client.query(
...
export async function actualizar(productoId, institucionId, data) {
  const { nombre, descripcion, precio, genera_ticket, activo, categoria_id } = data;
  const { rows } = await pool.query(
    `UPDATE productos
    ...
    [nombre, descripcion, precio, genera_ticket, activo, categoria_id, productoId, institucionId]
  );
  if (!rows[0]) throw createError(404, 'Producto no encontrado');
  return rows[0];
}
